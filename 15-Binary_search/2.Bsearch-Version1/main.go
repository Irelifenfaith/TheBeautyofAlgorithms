...
func bSearch(nums []int, value int) int {
	...
	for slow <= high {
		...
		if nums[mid] == value {
			...
			// 如果mid不等于0，但nums[mid]的前一个元素nums[mid-1]不等于value，
			// 那也说明nums[mid]就是我们要找的第一个值等于给定值的元素
			// 如果经过检查之后发现nums[mid]前面的一个元素nums[mid-1]也等于value，
			// 那说明此时的nums[mid]肯定不是我们要查找的第一个值等于给定值的元素。
